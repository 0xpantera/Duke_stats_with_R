...
posterior_a <- prior_a + sum(x)
posterior_b <- prior_b + length(x)
```
...
prior_alpha <- 5
prior_beta <- 1
x2 <- brfss$fruit_per_day

posterior_alpha <- prior_alpha + sum(x2)
posterior_beta <- prior_beta + length(x2)
```
...
prior_alpha_veg <- 5
prior_beta_veg <- 1
x3 <- brfss$vege_per_day

posterior_alpha_veg <- prior_alpha_veg + sum(x3)
posterior_beta_veg <- prior_beta_veg + length(x3)

qbeta(c(0.025, 0.975), shape1 = 5434, shape2 = 5001)
```
