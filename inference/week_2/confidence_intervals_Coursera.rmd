...
```{r set-seed}
set.seed(1282016)                 # make sure to change the seed
```
...
```{r describe-sample}
samp %>%
    summarise(mean_area = mean(area), median_area = median(area), IQR_area = IQR(area))
...
ci_99 <- ci_99 %>%
    mutate(capture_mu = ifelse(lower < mean(ames$area) & upper > mean(ames$area), "yes", "no"))
