...
```{r samp1-dist}
ames %>% 
    summarise(sampMean = mean(area), sampMed = median(area),
              s = sd(area), sampIQR = IQR(area),
              sampMin = min(area), sampMax = max(area),
              sampQ1 = quantile(area, 0.25), # First quantile, 25th percentile
              sampQ3 = quantile(area, 0.25)) # Third quantile, 75th percentile
```
...
```{r sampling-dist}
sample_means50 %>%
    summarise(n = n(), medianMeans = median(sample_means50$x_bar))
```
...
sample_means15 <- ames %>%
    rep_sample_n(size = 15, rep = 2000, replace = TRUE) %>%
    summarise(x_bar = mean(price))
...
sample_means150 <- ames %>%
    rep_sample_n(size = 150, rep = 2000, replace = TRUE) %>%
    summarise(x_bar = mean(price))
